...
`verification.json` records 44 transport-free Rust tests, 46 all-feature Rust tests, 57 controlled Python tests, the independent consumer, syntax package verification, package-asset inspection, and source hashes. It also records the initial test-fixture correction and classified local socket permission rerun. These new results are separate from historical upstream evidence.

The final receipt was regenerated after fixing Linux bridge response forwarding. The first Linux run and a diagnostic exposed an inherited `io::copy` pipe stall; the transport regression now requires a complete response before either peer closes. The rebuilt host passed the local contracts and another full native smoke. Across the initial and final extraction smoke runs there were two verified-reuse activations, zero fresh native compilations, and zero provider calls.
